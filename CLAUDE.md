...
11. **Admin management** — Owner-only **`/admin/jobs`**, **`/admin/reviews`**, **`/admin/companies`**: dark admin shell, jobs CRUD via server actions, soft delete (`jobs.status = deleted`), toggle open/closed, reviews moderation table (read from `public_reviews`, delete from `reviews`), and companies table with inline edit modal (name/industry/size/website/description via server actions). Gate: **`profiles.is_admin`** + **`requireAdmin()`** in **`lib/require-admin.ts`**. Not linked from public or user dashboard nav. **Access:** apply migrations `20260412120000_admin_profiles_is_admin_jobs_rls.sql` and `20260414123000_admin_reviews_companies_rls.sql`, run `update public.profiles set is_admin = true where lower(btrim(email)) = lower('you@example.com');` once, sign in, open any `/admin/*` route.
...
*Last updated: 2026-04-14 — Admin `/admin/reviews` and `/admin/companies` shipped with server actions (`app/admin/reviews/actions.ts`, `app/admin/companies/actions.ts`) and migration `20260414123000` for admin review delete/company update policies. Prior: admin `/admin/jobs`, migration `20260412120000`.*
